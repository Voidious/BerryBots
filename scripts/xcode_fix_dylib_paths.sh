...
FREETYPE_RPATH=${RPATH}/../Frameworks/freetype.framework/Versions/A/freetype
FREETYPE_BUNDLE_PATH=${EXEC_FRAMEWORK_PATH}/freetype.framework/Versions/A/freetype
...
for TARGET in ${SFML_LIBNAMES} ; do
  ...
  for TARGET2 in ${SFML_LIBNAMES} ; do
    ...
    install_name_tool -change ${RPATH}/${TARGET}.${SFML_VERSION}.dylib ${NEWTARGETID} ${LIBFILE2}
    install_name_tool -change ${RPATH}/${TARGET}.${SFML_MAJOR}.dylib ${NEWTARGETID} ${LIBFILE2}
    install_name_tool -change ${RPATH}/${TARGET}.dylib ${NEWTARGETID} ${LIBFILE2}
  done
  install_name_tool -change ${FREETYPE_RPATH} ${FREETYPE_BUNDLE_PATH} ${LIBFILE}
done
